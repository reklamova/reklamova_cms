...
return static function (array $container, PDO $pdo, array $module): array {
    ...
    $fallback = static function (string $slug) use ($repo, $siteName, $h): bool {
        ...
        foreach ($repo->all(true) as $item) {
            echo '<article class="card">' . (($item['value'] ?? '') !== '' ? '<div class="value">' . $h($item['value']) . '</div>' : '') . '<h2>' . $h($item['title']) . '</h2><p class="muted">' . $h($item['subtitle']) . '</p><p>' . TextFormatter::withLinks((string) ($item['description'] ?? '')) . '</p>';
            if (($item['file_url'] ?? '') !== '') {
